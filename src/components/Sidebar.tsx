...
export default function Sidebar({
  ...
  return (
    ...
    >
      ...
      {/* User Login & Cloud Sync Section */}
      <div className="p-4 border-t border-slate-200/80 dark:border-white/[0.05] bg-slate-50/50 dark:bg-slate-950/20 flex flex-col gap-2 relative z-10">
        {user ? (
          <div className="flex items-center justify-between gap-3 bg-white dark:bg-[#121a30] p-3 rounded-xl border border-slate-200 dark:border-white/[0.05] shadow-2xs">
            <div className="flex items-center gap-2.5 min-w-0">
              <img
                src={user.photoURL || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=80&fit=crop&q=80"}
                alt={user.displayName || "User"}
                referrerPolicy="no-referrer"
                className="w-8 h-8 rounded-full border border-slate-200 dark:border-white/10 object-cover flex-shrink-0"
              />
              <div className="min-w-0">
                <p className="text-xs font-bold text-slate-900 dark:text-white truncate">
                  {user.displayName || "PTE Student"}
                </p>
                <span className="text-[10px] text-slate-400 dark:text-slate-500 block truncate text-left">
                  Cloud Synced
                </span>
              </div>
            </div>
            <button
              onClick={onSignOut}
              className="text-xs font-bold text-rose-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 px-2 py-1 rounded-lg border border-transparent transition-all cursor-pointer bg-transparent"
              title="Sign Out"
            >
              Sign Out
            </button>
          ...
          <div className="bg-white dark:bg-[#121a30] p-3 rounded-xl border border-slate-200 dark:border-white/[0.05] shadow-2xs flex flex-col gap-2">
            <div className="text-left">
              <span className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wide block mb-1">
                Cloud Backups & Syncing
              </span>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-normal">
                Sign in to save progress and persist your reading coaching history across multiple browsers.
              </p>
            </div>
            <button
              onClick={onSignIn}
              className="w-full py-2 px-3 bg-slate-900 hover:bg-slate-800 dark:bg-slate-200 dark:hover:bg-white text-white dark:text-slate-900 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer border border-transparent shadow-2xs"
            >
